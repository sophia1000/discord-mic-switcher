...
BUILD_ID = "2026-01-02.shared-poll.shared-cooldown.freeze-aware"
...
class Settings:
    # Core behavior
    # Default requested: system starts ON
    system_enabled: bool = True

    # Timing / debouncing
    # NOTE: This build uses ONE shared polling interval and ONE shared cooldown.
    # - discord_poll_interval_ms is the shared poll tick for Discord + sync engine.
    # - global_action_lock_ms is the shared cooldown window for ALL outbound actions.
    # - ignore_own_change_ms is the shared "side effects" suppression window.
    enforce_interval_ms: int = 100  # legacy (kept for settings compatibility; not used)
    ignore_own_change_ms: int = 1000  # requested

    # Command pacing (prevents multi-toggle)
    # legacy (kept for compatibility; not used)
    discord_command_cooldown_ms: int = 1000
    vrc_command_cooldown_ms: int = 1000

    # Verify delays (give app time to update state before deciding to retry)
    # legacy (kept for compatibility; not used)
    discord_verify_delay_ms: int = 500
    vrc_verify_delay_ms: int = 500

    # NEW: Global shared lock after ANY outbound action (0 = auto)
    # Shared cooldown for ALL outbound actions (requested)
    global_action_lock_ms: int = 1000
    ...
    # Discord
    # Shared polling tick for Discord + sync engine (requested)
    discord_poll_interval_ms: int = 100
    ...
    vrc_press_ms: int = 80
    # default requested: extra release not needed
    vrc_extra_release: bool = False
...
class SharedState:
    ...
    # last successful poll time (not a "change" time)
    discord_last_update_at: float = 0.0
    # last time stable states changed
    discord_mute_changed_at: float = 0.0
    discord_deafen_changed_at: float = 0.0
    discord_running: bool = False
    discord_ready: bool = False  # UIA can read buttons/state
    discord_hung: bool = False

    # Enable
    system_enabled_ui: bool = False
    system_enabled_vrc: Optional[bool] = None
    system_enabled_ui_changed_at: float = 0.0
    system_enabled_vrc_changed_at: float = 0.0

    # mic-bridge effective (paused by deafen, closed apps, etc)
    mic_effective: bool = False
    pause_reason: str = ""

    # Shared action cooldown (countdown-based so it can pause when apps are frozen)
    action_locked: bool = False
    action_lock_remaining_ms: int = 0
    suppress_remaining_ms: int = 0
    last_action: str = ""
    last_action_at: float = 0.0

    # Engine freeze reporting (cooldown pauses while frozen)
    frozen: bool = False
    frozen_reason: str = ""

    # Freeze visibility (e.g., Discord "Not Responding")
    frozen: bool = False
    frozen_reason: str = ""
...
class StateStore:
    def __init__(self, enabled: bool):
        ...
        t = now()
        self.state = SharedState(system_enabled_ui=enabled, system_enabled_ui_changed_at=t)
...
# Detect a hung ("Not Responding") window.
...
class DiscordUiaReader:
    ...
    def is_hung(self) -> bool:
        """True if Discord window is "Not Responding" (process/UI thread hung)."""
        try:
            if self._hwnd is None:
                return False
            return bool(user32.IsHungAppWindow(wintypes.HWND(self._hwnd)))
        except Exception:
            return False
    ...
    def _on_osc(self, address: str, *args: Any) -> None:
        try:
            ...
            if self._toggle_aliases and tail in self._toggle_aliases:
                v = self._coerce_bool(args[0] if args else None)
                if v is not None:
                    self.store.update(system_enabled_vrc=v, system_enabled_vrc_changed_at=now())
                return
...
class DiscordPoller(threading.Thread):
    def __init__(self, settings: Settings, store: StateStore, reader: DiscordUiaReader, logger: logging.Logger):
        ...
        # Raw -> stable debouncing (helps prevent spurious UIA reads from causing toggles)
        self._raw_mute: Optional[bool] = None
        self._raw_deaf: Optional[bool] = None
        self._raw_mute_streak = 0
        self._raw_deaf_streak = 0

        self._stable_mute: Optional[bool] = None
        self._stable_deaf: Optional[bool] = None

        self._stable_needed = 3  # consecutive identical reads required
        self._last_good_read_at = 0.0
        self._stale_after_s = 2.5

        self._last_hung = False

    @staticmethod
    def _debounce(raw: Optional[bool], prev_raw: Optional[bool], streak: int) -> Tuple[Optional[bool], int]:
        if raw is None:
            return prev_raw, 0
        if prev_raw is None or raw != prev_raw:
            return raw, 1
        return prev_raw, streak + 1
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                hung = bool(self.reader.is_hung())
                if hung:
                    if not self._last_hung:
                        self.log.info("Discord window is not responding (hung) -> pausing reads")
                    self._last_hung = True
                    # When hung, mark unknown/!ready so the engine freezes cooldown and stops sending hotkeys.
                    self._raw_mute = None
                    self._raw_deaf = None
                    self._raw_mute_streak = 0
                    self._raw_deaf_streak = 0
                    self._stable_mute = None
                    self._stable_deaf = None
                    self.store.update(
                        discord_hung=True,
                        discord_ready=False,
                        discord_muted=None,
                        discord_deafened=None,
                        discord_last_update_at=t,
                    )
                else:
                    if self._last_hung:
                        self.log.info("Discord window recovered (no longer hung)")
                    self._last_hung = False

                    raw_mute, raw_deaf = self.reader.read_states()

                    self._raw_mute, self._raw_mute_streak = self._debounce(
                        raw_mute, self._raw_mute, self._raw_mute_streak
                    )
                    self._raw_deaf, self._raw_deaf_streak = self._debounce(
                        raw_deaf, self._raw_deaf, self._raw_deaf_streak
                    )
                    ...
                    if raw_mute is not None and self._raw_mute_streak >= self._stable_needed:
                        if self._stable_mute != raw_mute:
                            self._stable_mute = raw_mute
                            updates["discord_mute_changed_at"] = t
                            self.log.info("Discord stable mute -> %s", self._stable_mute)
                        accepted_any = True

                    if raw_deaf is not None and self._raw_deaf_streak >= self._stable_needed:
                        if self._stable_deaf != raw_deaf:
                            self._stable_deaf = raw_deaf
                            updates["discord_deafen_changed_at"] = t
                            self.log.info("Discord stable deafen -> %s", self._stable_deaf)
                        accepted_any = True

                    if accepted_any and (self._stable_mute is not None or self._stable_deaf is not None):
                        self._last_good_read_at = t

                    # If we can't get stable reads for too long, mark unknown so the engine pauses.
                    if self._last_good_read_at and (t - self._last_good_read_at) > self._stale_after_s:
                        if self._stable_mute is not None or self._stable_deaf is not None:
                            self.log.info("Discord state stale -> marking unknown until stable reads resume")
                        self._stable_mute = None
                        self._stable_deaf = None

                    updates["discord_muted"] = self._stable_mute
                    updates["discord_deafened"] = self._stable_deaf
                    updates["discord_ready"] = (self._stable_mute is not None and self._stable_deaf is not None)
...
class SyncEngine(threading.Thread):
    """Single-timer sync engine.

    Key rules (per your request):
    - ONE shared poll interval (discord_poll_interval_ms)
    - ONE global cooldown (global_action_lock_ms) after ANY outbound action
    - No separate verify delays. Verification happens when the cooldown expires.
    - Cooldown countdown PAUSES while Discord/VRChat is closed, UIA isn't readable,
      or Discord is "Not Responding" (hung window).
    """
    ...
    def __init__(self, settings: Settings, store: StateStore, vrc: VrchatOsc, logger: logging.Logger):
        ...
        self._vrc_lock = threading.Lock()
        self._vrc = vrc

        # shared countdown timers
        self.cooldown_remaining_ms: int = 0
        self.suppress_remaining_ms: int = 0
        self.pending: Optional[PendingAction] = None

        # last observed (for change detection)
        ...
        self.prev_ui_enabled: Optional[bool] = None
        self.prev_vrc_enabled: Optional[bool] = None

    def set_vrc(self, vrc: VrchatOsc) -> None:
        with self._vrc_lock:
            self._vrc = vrc

    def _vrc_ref(self) -> VrchatOsc:
        with self._vrc_lock:
            return self._vrc

    def _tick_ms(self) -> int:
        return max(50, int(self.s.discord_poll_interval_ms))

    def _cooldown_ms(self) -> int:
        # default requested: 1000
        return max(150, int(self.s.global_action_lock_ms))

    def _max_attempts(self) -> int:
        return max(1, int(self.s.max_attempts_per_sync))

    def _set_last_action(self, kind: str) -> None:
        self.store.update(last_action=kind, last_action_at=now())

    def _freeze_reason(self, st: SharedState, needs_discord: bool, needs_vrc: bool) -> str:
        if needs_discord:
            if not st.discord_running:
                return "Discord closed"
            if st.discord_hung:
                return "Discord not responding"
            if not st.discord_ready:
                return "Discord UI not ready"
        if needs_vrc:
            if not st.vrc_running:
                return "VRChat closed"
        return ""

    def _is_frozen(self, st: SharedState, pending: Optional[PendingAction]) -> Tuple[bool, str]:
        if pending is None:
            # we still consider the engine "frozen" for UI display when Discord UIA isn't readable,
            # but we don't block non-discord actions unless they're pending.
            if not st.discord_running:
                return True, "Discord closed"
            if st.discord_hung:
                return True, "Discord not responding"
            if not st.discord_ready:
                return True, "Discord UI not ready"
            if not st.vrc_running:
                return True, "VRChat closed"
            return False, ""

        needs_discord = pending.kind.startswith("dc_")
        needs_vrc = pending.kind.startswith("vrc_")
        reason = self._freeze_reason(st, needs_discord, needs_vrc)
        return (bool(reason), reason)

    def _begin_action(self, kind: str, target: bool, send_fn, optimistic: Optional[Dict[str, Any]] = None) -> None:
        """Send an outbound action and enter global cooldown."""
        try:
            ok = bool(send_fn())
        except Exception as e:
            self.log.warning("Action %s threw: %s", kind, e)
            ok = False

        self.pending = PendingAction(kind=kind, target=bool(target), attempts=1)
        self.cooldown_remaining_ms = self._cooldown_ms()
        # Treat all state updates during cooldown as side-effects.
        self.suppress_remaining_ms = max(self.suppress_remaining_ms, int(self.s.ignore_own_change_ms))
        self._set_last_action(kind)

        if optimistic:
            self.store.update(**optimistic)

        self.log.info("TX %s -> %s (ok=%s)", kind, bool(target), ok)

    def _verify_pending(self, st: SharedState) -> Optional[bool]:
        """Return True/False when verifiable; None when not verifiable yet."""
        if self.pending is None:
            return True

        kind = self.pending.kind
        target = bool(self.pending.target)

        if kind == "dc_mute":
            if st.discord_muted is None:
                return None
            return bool(st.discord_muted) == target
        if kind == "vrc_mute":
            if st.vrc_muted is None:
                return None
            return bool(st.vrc_muted) == target
        if kind == "dc_deafen":
            if st.discord_deafened is None:
                return None
            return bool(st.discord_deafened) == target
        if kind == "vrc_deafen_param":
            if st.vrc_deafen is None:
                return None
            return bool(st.vrc_deafen) == target
        if kind == "vrc_toggle_param":
            if st.system_enabled_vrc is None:
                return None
            return bool(st.system_enabled_vrc) == target
        return False

    def _retry_pending(self, st: SharedState) -> None:
        """Retry the pending action (if still needed) and re-enter cooldown."""
        if self.pending is None:
            return
        if self.pending.attempts >= self._max_attempts():
            self.log.warning("Pending %s failed after %s attempts; giving up.", self.pending.kind, self.pending.attempts)
            self.pending = None
            return
        ...
        # If already at target, clear without sending.
        ok = self._verify_pending(st)
        if ok is True:
            self.pending = None
            return

        # Send again only when we can read states.
        if kind == "dc_mute":
            if st.discord_muted is None:
                return
            if bool(st.discord_muted) == target:
                self.pending = None
                return
            send_fn = lambda: send_hotkey(self.s.discord_mute_hotkey, self.log)
        elif kind == "dc_deafen":
            if st.discord_deafened is None:
                return
            if bool(st.discord_deafened) == target:
                self.pending = None
                return
            send_fn = lambda: send_hotkey(self.s.discord_deafen_hotkey, self.log)
        elif kind == "vrc_mute":
            send_fn = lambda: self._vrc_ref().ensure_vrc_mute(target, st.vrc_muted)
        elif kind == "vrc_deafen_param":
            send_fn = lambda: self._vrc_ref().send_deafen_param(target)
        elif kind == "vrc_toggle_param":
            send_fn = lambda: self._vrc_ref().send_toggle_param(target)
        else:
            self.pending = None
            return
        ...
        try:
            ...
            self.log.warning("Retry %s threw: %s", kind, e)
            ok2 = False

        self.log.info("RETRY %s -> %s (attempt %s, ok=%s)", kind, target, self.pending.attempts, ok2)

    def _schedule_from_state(self, st: SharedState, enabled_effective: bool) -> None:
        """Pick at most ONE outbound action when not locked/suppressed."""

        # Sync system enable toggle (UI -> VRChat param) when configured.
        if self.s.vrc_toggle_param.strip() and st.vrc_running:
            ui_val = bool(st.system_enabled_ui)
            vrc_val = st.system_enabled_vrc
            if vrc_val is None or bool(vrc_val) != ui_val:
                # Prefer the side that changed most recently.
                ui_newer = st.system_enabled_ui_changed_at >= st.system_enabled_vrc_changed_at
                if ui_newer:
                    self._begin_action(
                        "vrc_toggle_param",
                        ui_val,
                        lambda: self._vrc_ref().send_toggle_param(ui_val),
                        optimistic={"system_enabled_vrc": ui_val, "system_enabled_vrc_changed_at": now()},
                    )
                    return

        # Deafen sync runs even when mic bridge is paused.
        deafen_sync_enabled = bool(self.s.discord_deafen_hotkey.strip()) and bool(self.s.vrc_deafen_param.strip())
        if deafen_sync_enabled and st.discord_running and st.vrc_running and st.discord_ready and not st.discord_hung:
            dc = st.discord_deafened
            vrc = st.vrc_deafen
            dc_changed = (dc is not None and dc != self.prev_dc_deafen)
            vrc_changed = (vrc is not None and vrc != self.prev_vrc_deafen)

            # Change-driven priority
            if dc_changed and dc is not None:
                if vrc is None or bool(vrc) != bool(dc):
                    self._begin_action(
                        "vrc_deafen_param",
                        bool(dc),
                        lambda: self._vrc_ref().send_deafen_param(bool(dc)),
                        optimistic={"vrc_deafen": bool(dc), "vrc_deafen_last_update_at": now()},
                    )
                    return
            if vrc_changed and vrc is not None:
                if dc is None or bool(dc) != bool(vrc):
                    self._begin_action(
                        "dc_deafen",
                        bool(vrc),
            ...
            # Enforce match if both known and mismatched
            if dc is not None and vrc is not None and bool(dc) != bool(vrc):
                # choose newer change
                dc_newer = st.discord_deafen_changed_at >= st.vrc_deafen_last_update_at
                if dc_newer:
                    self._begin_action(
                        "vrc_deafen_param",
                        bool(dc),
        ...
        # Mic opposite bridge only when effective.
        if not enabled_effective:
            return

        if not (st.discord_running and st.vrc_running and st.discord_ready and not st.discord_hung):
            return
        if st.discord_deafened:
            return

        dc_m = st.discord_muted
        vrc_m = st.vrc_muted
        if dc_m is None or vrc_m is None:
            return

        dc_changed = (dc_m != self.prev_dc_mute)
        vrc_changed = (vrc_m != self.prev_vrc_mute)

        # Change-driven priority
        if dc_changed:
            desired_vrc = not bool(dc_m)
            if bool(vrc_m) != bool(desired_vrc):
                self._begin_action(
                    "vrc_mute",
                    bool(desired_vrc),
                    lambda: self._vrc_ref().ensure_vrc_mute(bool(desired_vrc), st.vrc_muted),
                )
                return
        if vrc_changed:
            desired_dc = not bool(vrc_m)
            if bool(dc_m) != bool(desired_dc):
                self._begin_action(
                    "dc_mute",
                    bool(desired_dc),
        ...
        if bool(dc_m) != bool(desired_dc):
            # pick newer change
            dc_newer = st.discord_mute_changed_at >= st.vrc_last_update_at
            if dc_newer:
                self._begin_action(
                    "vrc_mute",
                    not bool(dc_m),
                    lambda: self._vrc_ref().ensure_vrc_mute(not bool(dc_m), st.vrc_muted),
                )
            else:
                self._begin_action(
                    "dc_mute",
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            # Enabled state (VRChat toggle param optional)
            ...
            enabled_effective = bool(vrc_enabled) if (vrc_toggle_configured and vrc_enabled is not None) else ui_enabled

            # Compute pause reason for mic bridge.
            pause_reason = ""
            if not st.discord_running:
                pause_reason = "Discord closed"
            elif not st.vrc_running:
                pause_reason = "VRChat closed"
            elif st.discord_hung:
                pause_reason = "Discord not responding"
            elif not st.discord_ready:
                pause_reason = "Discord UI not ready"
            elif st.discord_deafened:
                pause_reason = "Discord deafened"

            mic_effective = enabled_effective and (pause_reason == "")

            # Freeze detection for timers
            frozen, frozen_reason = self._is_frozen(st, self.pending)

            # countdowns (pause while frozen)
            if not frozen:
                if self.cooldown_remaining_ms > 0:
                    self.cooldown_remaining_ms = max(0, self.cooldown_remaining_ms - tick_ms)
                if self.suppress_remaining_ms > 0:
                    self.suppress_remaining_ms = max(0, self.suppress_remaining_ms - tick_ms)

            # Update UI-facing state
            self.store.update(
                mic_effective=mic_effective,
                pause_reason=pause_reason,
                action_locked=(self.cooldown_remaining_ms > 0),
                action_lock_remaining_ms=int(self.cooldown_remaining_ms),
                suppress_remaining_ms=int(self.suppress_remaining_ms),
                frozen=frozen,
                frozen_reason=frozen_reason,
            )

            # If locked, verify when cooldown ends.
            if self.pending is not None:
                if self.cooldown_remaining_ms <= 0 and not frozen:
                    ok = self._verify_pending(st)
                    if ok is True:
                        self.log.info("OK %s == %s", self.pending.kind, bool(self.pending.target))
                        self.pending = None
                    elif ok is False:
                        self._retry_pending(st)
                    else:
                        # cannot verify yet (unknown state), wait
                        pass
            else:
                # Not pending: if not locked and not suppressing, schedule one action.
                if self.cooldown_remaining_ms <= 0 and self.suppress_remaining_ms <= 0:
                    self._schedule_from_state(st, enabled_effective)
            ...
            self.prev_ui_enabled = bool(st.system_enabled_ui)
            self.prev_vrc_enabled = st.system_enabled_vrc
...
class App(tk.Tk):
    ...
    def _restart_vrc(self) -> None:
        ...
        try:
            self.vrc = VrchatOsc(self.s, self.store, self.log)
            ok = self.vrc.start()
            # IMPORTANT: make sure the running SyncEngine uses the new VrchatOsc instance
            try:
                if self.sync_engine:
                    self.sync_engine.set_vrc(self.vrc)
            except Exception:
                pass
            if not ok:
    ...
    def _on_toggle(self) -> None:
        ...
        self.store.update(system_enabled_ui=enabled, system_enabled_ui_changed_at=now())
        self.s.system_enabled = enabled

        # VRChat toggle param (if configured) is synchronized by the SyncEngine.
        ...
        try:
            ...
            if self.s.vrc_toggle_param.strip() and st.system_enabled_vrc is not None:
                ...
                if bool(self.var_enabled.get()) != v:
                    ...
                    self.store.update(system_enabled_ui=v, system_enabled_ui_changed_at=now())
        except Exception:
            pass
        self.after(220, self._sync_ui_toggle_from_vrc)
    ...
    def _refresh_state_ui(self) -> None:
        ...
        if st.discord_hung:
            apps.append("Discord: NOT RESPONDING")

        if st.action_locked:
            apps.append(f"Lock: ON ({int(st.action_lock_remaining_ms)}ms)")
        else:
            apps.append("Lock: OFF")

        if st.suppress_remaining_ms > 0:
            apps.append(f"IgnoreOwn: {int(st.suppress_remaining_ms)}ms")
        if st.frozen:
            apps.append(f"Frozen: {st.frozen_reason}")
